...
const listkey = ["apirey", "FtBot", "ditofficial"];
...
router.get('/info/wikiid', async (req, res, next) => {
        var Apikey = req.query.apikey,
	...
	if(listkey.includes(Apikey)){
       ...
       fetch(encodeURI(`http://kocakz.herokuapp.com/api/edu/idwiki?query=${search}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/info/cerpen', async (req, res, next) => {
        var Apikey = req.query.apikey
...
router.get('/info/cersex', async (req, res, next) => {
        var Apikey = req.query.apikey
